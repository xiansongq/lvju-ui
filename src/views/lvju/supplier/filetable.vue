  ...
  <div v-if="agentType==2">
      ...
      <el-table :data="supattchList">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          ...
          <el-tooltip content="下载" placement="top">
            <el-button link type="primary" icon="Edit" @click="handleDownload(scope.row)" v-hasPermi="['lvju:supplier:edit']"></el-button>
          </el-tooltip>
    ...
    <div v-if="stype==2">
      ...
      <el-table :data="supattchList">
        ...
        <el-table-column label="操作" align="center" prop="sdnum">
          <el-tooltip content="下载" placement="top">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['lvju:supplier:edit']"></el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top">
            <el-button link type="primary" icon="Delete" @click="handleUpdate(scope.row)" v-hasPermi="['lvju:supplier:edit']"></el-button>
          </el-tooltip>
        </el-table-column>
      </el-table>
    </div>
...
import { listSupattch, getSupattch, delSupattch, addSupattch, updateSupattch ,allSupattch} from '@/api/lvju/supattch';
  ...
  for (var i = 0; i < agentpList.value.length; i++) {
    ...
    for (var j = 0; j < supattchList.value.length; j++) {
      if (agentpList.value[i].typeValue === supattchList.value[j].typeValue) {
        ...
        console.log(supattchList.value[j].typeValue, agentpList.value[i].typeValue);
        flag = 1;
      }
    }
...
/* 下载文件 */
const handleDownload =async (row?: InfoVo) => {
    if(row?.id===null){
